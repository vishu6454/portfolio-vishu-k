      ...
      <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-8 lg:gap-12 items-center relative z-10">
        ...
        >
          <div className="relative mx-auto max-w-sm">
            <div className="absolute -inset-1 bg-linear-to-r from-purple-600 to-emerald-600 rounded-2xl blur-lg opacity-50" />
            <motion.img 
              src={profileImg} 
              className={`w-full rounded-2xl shadow-xl mx-auto border-4 relative z-10 ${
            ...
            {/* Floating elements */}
            <motion.div
              className="absolute -top-3 -left-3 sm:-top-4 sm:-left-4 bg-linear-to-r from-purple-600 to-pink-600 text-white p-3 sm:p-4 rounded-xl shadow-lg z-20"
            ...
            <motion.div
              className="absolute -bottom-3 -right-3 sm:-bottom-4 sm:-right-4 bg-linear-to-r from-emerald-600 to-cyan-600 text-white p-3 sm:p-4 rounded-xl shadow-lg z-20"
              initial={{ opacity: 0, scale: 0, rotate: 45 }}
              animate={isInView ? { opacity: 1, scale: 1, rotate: 0 } : {}}
        ...
        >
          ...
          >
            {qualities.map((quality, index) => (
              <motion.div 
                key={index}
                className={`flex items-center gap-3 p-3 rounded-xl shadow-md hover:shadow-lg transition-all backdrop-blur-sm ${
          ...
          >
            {stats.map((stat, index) => (
              <motion.div
                key={index}
                className={`p-3 rounded-xl text-center shadow-md hover:shadow-lg transition-all backdrop-blur-sm ${
                  isDark ? "bg-slate-800/50" : "bg-white/80"
                }`}
          ...
          <motion.a 
            href="#services"
            className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-linear-to-r from-purple-600 to-emerald-600 text-white font-semibold shadow-lg hover:shadow-purple-500/25 hover:scale-105 transition-all group"
            whileHover={{ y: -2 }}
            whileTap={{ scale: 0.95 }}
